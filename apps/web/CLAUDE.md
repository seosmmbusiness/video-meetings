...
- `src/app/` — App Router routes. `layout.tsx` is the root layout, `page.tsx` the home route (a Server Component). `register/`, `login/` — registration/login pages against `apps/api`'s auth endpoints; `actions/auth.ts` — the Server Actions backing them; see `.claude/modules/module-web-auth.md`.
- `src/lib/` — `auth-api.ts` (server-only fetch client for `apps/api`'s `/auth/*`) and `session.ts` (the `httpOnly` session cookie: read/write/clear plus JWT claim decoding); see `.claude/modules/module-web-auth.md`.
- `e2e/` — Playwright e2e specs, run against a dev server Playwright starts itself (see `playwright.config.ts`). `auth.spec.ts` additionally requires `apps/api` + Postgres to already be running (Playwright only manages the `apps/web` dev server) — see `.claude/modules/module-web-auth.md`.
