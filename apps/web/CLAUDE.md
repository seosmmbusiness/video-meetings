...
- `src/components/files/file-uploader.tsx` — the Client Component driving multi-file upload (progress, cancel, retry) from the meeting page; `file-preview.tsx` — the Client Component rendering a file in place (video/audio/image/PDF); see `docs/modules/module-web-meeting-files.md`.
...
- `e2e/` — Playwright e2e specs, run against a dev server Playwright starts itself (see `playwright.config.ts`). `auth.spec.ts`, `home.spec.ts`, `meeting-page.spec.ts`, `meeting-file-upload.spec.ts` and `meeting-file-preview.spec.ts` additionally require `apps/api` + Postgres to already be running (Playwright only manages the `apps/web` dev server) — see `docs/modules/module-web-auth.md` and `docs/modules/module-web-meeting-files.md`.
