...
Two smaller decisions. The **confirmation match runs in the action**, not in the browser, because a Server Action is reachable by a direct POST and a browser-side gate would only cover the rendered form — FINAL justified it as a no-JavaScript guarantee, which this form does not actually offer: `action` is bound to a client wrapper, the same shape `NameForm` uses to keep what was typed through a refusal, so it does not progressively enhance. The check is right; the reason it is right is the direct POST. For the same reason the new-password field carries no `pattern` and no `maxLength` — a client-side rule would swallow the value before `apps/api` could name the rule it broke. The fields are controlled, as `NameForm`'s is, and are emptied on success so neither password lingers in the DOM. One refusal is deliberately **not** passed through verbatim: the route's own throttle (S-4) answers `ThrottlerException: Too Many Requests`, an exception class name, so the action says it in words instead — the only place the web app writes its own copy over the API's.
