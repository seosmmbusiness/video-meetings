...
Three mechanics were forced by the setup and are worth remembering: the default environment is jsdom with a `// @vitest-environment node` docblock for specs needing real Node globals; `describe`/`it`/`expect` are imported explicitly rather than made global, so nothing depends on ambient types; and `server-only` is aliased to an inert stub, because the real package throws unless the resolver applies React's `react-server` condition — which Vitest can't do without swapping React itself for its server build, so without the alias no `src/lib` module is importable from a spec.
