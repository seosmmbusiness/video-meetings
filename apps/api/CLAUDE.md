...
- `src/app.module.ts` / `app.controller.ts` / `app.service.ts` — root module/controller/service; `AppModule` wires up `ConfigModule` (global), `PrismaModule`, and feature modules.
...
- `src/auth/` — email/password auth: `AuthController` (`POST /auth/register`, `POST /auth/login`), `AuthService` (bcrypt hashing/verification, JWT issuance via `@nestjs/jwt`), `dto/` (`RegisterDto`, `LoginDto`, `AuthResponseDto`).
...
Postgres-backed email/password auth is implemented: `POST /auth/register` and `POST /auth/login`, both returning `{ accessToken }` (JWT via `@nestjs/jwt`, secret from `JWT_SECRET`/`JWT_EXPIRES_IN` in the root `.env`). Passwords are hashed with bcrypt; email format and password complexity (≥8 chars, upper+lower+digit) are enforced via `class-validator`; registration requires `consentToTerms: true`. Prisma (`User` model) is wired to Postgres — see the Database section above for generator/driver-adapter specifics. Redis is still unused. Update this file as more domain modules land.
