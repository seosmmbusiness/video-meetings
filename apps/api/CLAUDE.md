...
- `src/app.module.ts` / `app.controller.ts` / `app.service.ts` — root module/controller/service; `AppModule` wires up `ConfigModule` (global, reading the monorepo-root `.env`), a global `ThrottlerGuard` (`@nestjs/throttler`, 20 requests/60s by default, tracked per caller by the hashed bearer token rather than by socket; the baseline — and only the baseline — is configurable via `THROTTLE_LIMIT`/`THROTTLE_TTL_MS`, see `src/config/throttler.config.ts`), `CqrsModule.forRoot()` (once, app-wide), `ScheduleModule.forRoot()` (backing the files purge cron), `PrismaModule`, and the feature modules.
...
- `src/auth/` — authentication orchestration only: register/login flow, JWT issuance (through one `IssueAccessTokenCommand`, so the claim set — `ver` included — is written in a single place) and verification (the guard refuses a token whose `ver` no longer matches the account's `tokenVersion`), rate limiting. Delegates user persistence to `src/users` and password hashing/verification to `src/credentials`, via CQRS (see Conventions below). Architecture + function reference: `docs/modules/module-api-auth.md` (see root `CLAUDE.md`'s Module documentation section); the JWT verification guard/strategy used to protect routes is documented in `docs/modules/module-api-meetings.md` (added alongside that module, as its first consumer).
- `src/users/` — user persistence (Prisma `User` model): creation, lookup by email or id, the display-name update, the avatar-columns update and the password update that bumps `tokenVersion` in the same statement, exposed only via CQRS commands/queries. Architecture + function reference: `docs/modules/module-api-users.md`.
...
- `src/profile/` — the signed-in caller's own self-service routes (`GET`/`PATCH /profile`, `PATCH /profile/password`, and `POST`/`GET`/`DELETE /profile/avatar`), guarded and resolving their subject from the token alone, answering an explicit response DTO that never carries the avatar's storage key, the password hash or the revocation counter. Architecture + function reference: `docs/modules/module-api-profile.md`.
...
- The global throttler tracks callers by the hashed bearer token rather than by socket — keyed on
  the token itself, so the header spellings `passport-jwt` accepts all share one bucket — and its baseline (20/60 s unset, as in production) reads `THROTTLE_LIMIT`/`THROTTLE_TTL_MS` so a browser e2e run can widen it; unusable values fall back to the default instead of widening it.
