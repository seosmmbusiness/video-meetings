...
- `profile.controller.ts` — `@UseGuards(JwtAuthGuard)` on the whole controller; every route takes its subject from `@CurrentUser()` and delegates to the service. Fully annotated for Swagger (`@ApiTags('profile')`, `@ApiBearerAuth()`, 200/201/400/401/404 plus `@ApiConsumes('multipart/form-data')`, an `@ApiBody` multipart schema and one `@Api*Response` per refusal status — `413` and `415` — on the upload route). Per-route throttles: `{ limit: 240, ttl: 60_000 }` on the byte read, `{ limit: 30, ttl: 60_000 }` on the avatar write and delete — the write's row is S-5's rate control; the read's is the ordinary byte-route baseline.
