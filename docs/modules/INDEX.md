...
- [apps/api profile module](module-api-profile.md) — `src/profile`: the signed-in caller's own `GET`/`PATCH /profile` and the three `/profile/avatar` routes, subject taken from the token alone, answering an explicit five-key DTO.
