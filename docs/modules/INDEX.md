...
- [apps/web profile page](module-web-profile.md) — `app/profile`, `app/actions/profile.ts`, `app/api/profile/avatar`, `components/profile`, `lib/profile-api.ts`, `lib/avatar-limits.ts`: the account's email/name/avatar, the name form behind a Server Action, the avatar upload/replace/remove behind a same-origin byte proxy, and the dashboard's greeting and avatar mark.
