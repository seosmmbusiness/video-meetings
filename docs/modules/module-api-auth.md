...
- Global `ThrottlerGuard` (set up in `AppModule`, not this module) applies a default 20 req/60 s to every route, keyed by `trackerFromRequest` (`src/config/throttler.config.ts`): a hash of the bearer token when the request carries one, the caller's IP when it doesn't. `login` overrides that with a stricter `@Throttle({ default: { limit: 10, ttl: 60_000 } })` since it's the brute-force target.
- **Known gap on the two unauthenticated routes.** `ThrottlerGuard` is an `APP_GUARD`, so it runs before anything verifies the token — and on `POST /auth/register` and `POST /auth/login` nothing ever does. A caller who sends a different `Authorization` header on each request therefore keys a fresh bucket each time and never meets login's `limit: 10`. Every guarded route is unaffected: a request that reaches one carries a token the strategy accepted, so its bucket is fixed (which is why `PATCH /profile/password`'s S-4 control holds). Closing it means keying `/auth/*` by IP regardless of the header, which needs the `ExecutionContext` the tracker doesn't currently get.
