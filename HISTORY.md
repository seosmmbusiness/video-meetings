...
### 2026-08-30 — The engine's weights are provisioned, and its isolation is a compose overlay

Two pieces of infrastructure the transcription feature needs, both outside `apps/api` (MT 1.6).

**`npm run whisper:models`** downloads `ggml-${WHISPER_MODEL}.bin` once into the gitignored `.data/whisper-models/` and verifies it against the SHA1 whisper.cpp publishes. It exists because AC-12 forbids a lookup leaving the machine during a run, so the weights have to be there before the first one starts — the rejected alternatives were letting the engine fetch on first use (which is what disqualified the other engine image) and baking them into a locally-built image (a Dockerfile and a build step for what a read-only mount does). The SHA1 gate is not ceremony: this is the feature's only fetch from a general-purpose host, and unverified bytes would surface as a quietly worse transcript rather than as an error. The download lands under a `.part` name and is renamed only after it passes, so an interrupted run can never look provisioned, and a file already on disk is re-verified rather than trusted. `WHISPER_MODEL_SHA1` covers models the script has no checksum pinned for, so raising the model never means skipping the check.

**`docker-compose.offline.yml`** is AC-12's structural half: it redeclares `whisper_net` as `internal: true` and drives the engine from a one-shot `curl` container on that network, because a published port does **not** work on an internal network (moby#36174) — the topology that denies the engine egress is the same one that makes it unreachable from the host, so the proof has to run from inside. It drops the publish with `ports: !reset null` for that reason, and is therefore an overlay that is never the topology `apps/api` runs against.

**`WHISPER_MODELS_DIR` was added for a failure this machine found.** Docker Desktop bind-mounts only the directories its settings list (`/home` here), and a checkout outside them — this one lives on `/run/media` — mounts as an **empty** directory rather than refusing, which the engine reports as `failed to open '/models/ggml-tiny.bin'` and Compose reports as a healthy start followed by a restart loop. The variable moves the mount and the provisioning script together and defaults to the path the plan fixes, so the alternative — adding the checkout to Docker Desktop's file sharing — stays open and neither is imposed. The stock image itself runs fine as `user: "1000:1000"`, which research §8 had left open.

### 2026-08-24 — One home per rule: the CLAUDE.md files deduplicated

The four `CLAUDE.md` files and `README.md` had grown by accretion until the same fact was stated in up to five places — the test tiers, the npm scripts, the e2e throttle headroom, "Redis is optional", the Ralph loop, the git hooks, the env vars — and the copies had started to disagree: the root file still described the throttler as tracking callers by IP after 2026-08-23 keyed it on the bearer token, its guard-bash list marked `gh pr merge` as always refused when the hook refuses it only inside a loop link, and `apps/api/CLAUDE.md` gave a test command that ran from no directory. Two costs followed. A contradiction between copies is a coin toss for the model reading them, and the root file `@`-imported the whole README into every session — script tables and setup prose paid for on every turn.

Each fact now has one home and pointers elsewhere: the tiers, the order and the gates in the root Testing section; scripts nowhere but `package.json` (`npm run` prints them); env vars in `.env.example` and README's Database section; the Ralph loop in `docs/ralph-loop.md`; per-module one-liners in `docs/modules/INDEX.md` alone, so the app files keep only the gotchas their code doesn't say. The README import is gone — it stays the human setup doc, read on demand. The root file gained what nothing stated before: a precedence line (the rules outrank the style of existing code), a **Working with context** section (locate before reading, never load generated content, delegate wide reads, quiet commands), the Naming / Size / Dependencies / Refactoring rules, and an **Optional tooling** section — the plugin, the practice skills, the agents and the Playwright MCP are all things a session may not have, and every instruction that names one now says what to do without it. The plugin's own release log moved from `plugins/bldprj/CLAUDE.md` into `plugins/bldprj/CHANGELOG.md` (2.4.1) for the same reason this file exists: a Status section is for the present.

### 2026-08-23 — Three review agents gate every commit

`.claude/settings.json` now runs three `PreToolUse` agent hooks before any `git commit` the session issues — a security review, a correctness review and a test-coverage review, each reading its doctrine from the linked `.claude/agents/*.md` (security-analyst, code-reviewer, test-designer) — and any of them can block the commit. Each blocks only on what it can name — a reachable vulnerability, a defect with a wrong result, a behaviour change with no spec at the tier that could prove it — and a docs-only, config-only or deliberately red test-only commit passes, because this project commits red states on purpose. Because they are Claude Code hooks rather than a husky step, they see the diff inside the session that wrote it, with the project's own CLAUDE.md files in context, and a `git commit` from a terminal meets `.husky/pre-commit`'s lint alone. The gate depends on the agent symlinks: without them each hook still reviews, on a generic prompt. Landed in PR #192 without a log line; recorded here a day late.

### 2026-08-23 — The loop closes a feature out, and only the link in flight may move it

Four changes to `.claude/ralph/`, all of them paid for by the same run — `user-profile`, six phases, sixty-four sessions.

**The close-out is a stage now.** The chain used to end at `done`: it ran `/bldprj:close-feature`, was told in so many words to leave the PR for a person, and deactivated the run before the session even started. So a feature the loop had built end to end still needed somebody to merge one docs-only PR, delete twelve branches on `origin` and fix a path. `done` now opens the close-out PR and ends; `closeout-merge` re-runs both layers' suites, the build and the docs linter against it and merges on a clean sweep; `finished` ends the run without a halt. The close-out gate is the widest of the three on purpose — it is the last thing that runs before the acceptance criteria are archived, and it re-proves what `close-feature` claimed rather than trusting the session's own account of it. Two things the archive breaks were fixed with it: `readMs` falls back to `docs/archive/<slug>/` when the MS file has moved under the chain's feet, and the session contract reads the FINAL plan from `sources.final` instead of the slug that was hard-coded into it.

**Only the link in flight may decide what happens next.** `advance()` treats its caller as the link that just ended — it stamps `endedAt` and checks the record that link promised. Nothing enforced that the caller _was_ that link, and twice in one evening it was not: a `--resume` rebuilt the state without the `link` record, so a session still working became invisible; an orphaned session's Stop hook then arrived, was read as the current task finishing, failed the `ralph:done` check and spawned a second agent onto the same task, the same branch and the same working tree. Two agents editing `profile-api.ts` corrupted it mid-write. `linkOwner()` now answers one question — is this caller the link the chain is waiting on — by matching the hook's `session_id` against the id in the link's own log, or the step's pid against the process. A resume carries `link`, `closeout`, `stash` and `selection` across rather than dropping them, and refuses to advance at all while a link is alive. The blanket rule that made this hard to see from outside is gone: a chain that looks healthy while two sessions fight over one tree is the worst failure mode this loop has.

**A resume no longer refuses a dirty tree — it stashes it and hands it on.** Refusing meant a person untangling the leftovers of every killed session by hand; carrying on over them meant a session inheriting edits it never made. The leftovers now go into a named stash, untracked files included, and exactly one session — the one that inherits that tree — is told the stash exists, which files it touched, and to judge whether it belongs to its own task before applying or ignoring it. Never dropping it is part of the contract.

**When a run ends, the loop asks what else is open.** `.claude/ralph/pick.js` surveys open milestones, then phases the plans hold that GitHub does not, then features whose phases have all settled but which were never archived — in that order, because a milestone is work already cut into issues and a plan's phase is work that still has to be. Issues with neither a milestone nor a plan are listed and never offered: the loop's contract is a phase's Done when and Verified by, and an issue with neither gives a session nothing to work against. The pick is saved into the run's overrides, so a run given one milestone stops when that milestone settles instead of walking into the next phase, and a resume keeps working what was picked. The dashboard shows the same survey with a Start button, and splits stopping into what it always meant — stop when the link ends, stop now, stop after this phase.

One deliberate loosening came with it: `.claude/hooks/guard-bash.js` still refuses to delete branches, except a branch whose name is the pipeline's own (`feature/<slug>-phase-<N>`, `chore/<slug>-phase-<N>-done`, `chore/<slug>-closeout`) **and** which the base branch already contains. That is close-out step 7 and nothing else; an unmerged branch may still be the only copy of a phase, and is still refused.
